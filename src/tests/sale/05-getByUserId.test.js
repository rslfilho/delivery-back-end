...
describe('GET/sales/user/:userId', () => {
  ...
  describe('Em caso de erro interno', () => {
    let response;
    before(async () => {
      stub(saleService, 'getByUserId').rejects();
      response = await chai.request(app)
        .get('/sales/user/3')
        .set('Authorization', token)

      await saleService.getByUserId.restore();
    });
  ...
  describe('É possível retornar a venda com token válido e ID existente', () => {
    let response;
      ...
      it('o objeto possui as propriedades "id", "seller", "totalPrice", "deliveryAddress", "deliveryNumber", "saleDate", "status" e "products', () => {
        expect(response.body[0]).to.have.all.keys('id', 'seller', 'totalPrice', 'deliveryAddress', 'deliveryNumber', 'saleDate', 'status', 'products');
      });
  });
});
