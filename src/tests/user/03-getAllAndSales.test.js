...
describe('GET/users/sales', () => {
  ...
  describe('Em caso de erro interno', () => {
    let response;
    before(async () => {
      stub(userService, 'getAllAndSales').rejects();
      response = await chai.request(app)
        .get('/users/sales')
        .set('Authorization', token)

      await userService.getAllAndSales.restore();
    });
  ...
  describe('Não é possível listar usuários e vendas', () => {
    ...
    describe('com um usuário não autorizado', () => {
      let response;
      before(async () => {
  ...
  describe('É possível listar usuários e vendas com token válido', () => {
    let response;
      before(async () => {
        response = await chai.request(app)
        .get('/users/sales')
      ...
      it('o objeto possui as propriedades "id", "name", "email", "role", "userOrders" e "userSales"', () => {
        expect(response.body[0]).to.have.all.keys('id', 'name', 'email', 'role', 'userOrders', 'userSales');
      });
  });
});
