...
describe('DELETE/users/:id', () => {  
  ...
  describe('Em caso de erro interno', () => {
    let response;
    before(async () => {
      stub(userService, 'remove').rejects();
  ...
  describe('Não é possível deletar usuário', () => {
    ...
    describe('com um usuário que não pode deletar usuários', () => {
      let response;
      before(async () => {
  ...
  describe('É possível deletar usuário com dados corretos e token válido', () => {
    let response;
    before(async () => {
      response = await chai.request(app)
      .delete('/users/4')
    ...
    it('a propriedade "message" possui o texto "User deleted"', () => {
      expect(response.body.message).to.be.equal('User deleted');
    });
  });
});
